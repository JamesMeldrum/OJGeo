...
def main():
    import DataCrawler
    masterList = createSearchTerms()
    i=-1
    for category in masterList:
        i+=1
        for seller_type in category:
            dbname = str(i)+"_"+str(escape_input(seller_type))+".db"
            geoCrawler = DataCrawler.Crawler(dbname)
            page=['http://www.yellowpages.com.au/search/listings?region=australia&showAllLocations=false&includeSurroundingSuburbs=false&visitedIAPages=1&referredBy=www.yellowpages.com.au&eventType=pagination&selectedViewMode=list&surroundingSuburbsModifiedByUser=false&visitedTMPages=1&stateId=9&context=businessTypeSearch&clue=',escape_input(seller_type),'&serviceAreaModifiedByUser=false&pageNumber=',1,'&showAllCandidates=false&lruPageNumber=1&locationClue=All+States&serviceArea=false&locationText=All+States&searchMode=type&tmPageNumber=1']
            geoCrawler.testCrawl(page,escape_input(seller_type))
    return

def createSearchTerms():
    mlist =[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
    #mlist.append("auto")
...
def escape_input(input_string):
    ...
    input_string = input_string.replace("'","%27")
